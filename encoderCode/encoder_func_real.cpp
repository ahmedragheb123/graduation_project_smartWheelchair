...
double Kp = 0.5, Ki = 1.2, Kd = 0; // fist trial kp = 0.5 Ki=1.2, Kd=0,   second trial   kp = 0.5, ki = 1.5 kd = 0
...
void pid_controller(double setpoint, int dir1, double setpoint_2, int dir2)
{
  ...
  //{
    ...
    myPID.Compute();
    setmotor(dir1, Output, pwm, int1);

    Input_2 = (double)(calculating_rpm_2());
    myPID_2.Compute();
    setmotor(dir2, Output_2, pwm_2, int1_2);
 //    }
}
float rpmreq()
{
